...
num=100;    % Number of cities to be visited by a salesman
...
nind=200;    % Size of a chromosome population
...
disp('Hit any key to display the best route found in the initial chromosome population.')
pause

[a b]=min(ObjV);
figure('name','The best route found in the initial population');
plot(city_location(:,1),city_location(:,2),'.r','markersize',25)
...
for m=1:(ngener/n_show)
  ...
  %     [ans,Index]=(fitness)
     ...
     % newchrom=chrom(selind,:);
      ...
      % tournament=fitness(rand(1,numsel))
 ...
 % selind
   ...
   %    if i=1
    %       sel3=[]
     ...
     k= randperm(size(fitness,1));
     k=k(1:5);
     
     mnew = sort(fitness(k(1:5),:),'descend');
 
 %     mnew = (fitness(k(1:5),:))
      selfix(i)=mnew(1) ;    % to here is fine
      [sel3(i)]=k(1) ;         % to here is fine
...
%          selind2=find(fitness>.22)

        %       newchrom=chrom(selind,:);
                 newchrom=chrom(sel3,:);
      
      %    [sel2,j]=find(selfix >0)
     
 %    [sel,j]=find(mnew(1));
     % for i=1,numsel
 %      k(i,:)= randperm(size(fitness,1))
  %     mnew(i,:)= sort(fitness(k(1:5),:),'descend')
   %    sel(i,:)= mnew(i,1)
 %end
   end
   ...
   %      i
       if rand(1)<Pm
        % points=(round(rand(floor(numsel/2),1).*(ngenes-1))+1)';
         ...
         %         i
          %   movinggene=newchrom(i,points(1)+1)
   %newchrom(i,:)      
%newchrom(i,points(1)+1:points(2))
    ...
    if param<0.4
        newchrom(i,:)=[newchrom(i,1:points(1)),...
        circshift((newchrom(i,points(1)+1:points(2))),[0,-1]),...
        newchrom(i,points(2)+1:ngenes)];
    elseif param<0.8
        newchrom(i,:)=[newchrom(i,1:points(1)),...
        circshift((newchrom(i,points(1)+1:points(2))),[0,+1]),...
 ...
 %   newchrom
%   newchrom(i,points(2))=movinggene                      
%newchrom(i,:)
%newchrom(i,points(1)+1:points(2))
%circshift((newchrom(i,points(1)+1:points(2))),[0,-1])
...
%points(2)           
       ...
       %      temp=newchrom(i,points(1)+1:points(2)) 
             
             %newchrom(i,:)=[newchrom(i,1:points(1)),...
             %circshift((newchrom(i,points(1)+1:points(2))),-1),...
             %newchrom(i,points(2)+1:ngenes)];
...
%        for i=1:numsel
 %        i
  %     if rand(1)<Pm
   ...
   %      disp('replacing part of population')
   %      nind;
    %     numsel;
    %     pause
   ...
   % Plotting the best route found in the current population
